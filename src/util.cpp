...
const char * const BITCOIN_CONF_FILENAME = "nexnexcoin.conf";
const char * const BITCOIN_PID_FILENAME = "nexnexcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "nexnexcoin" is a composite category enabling all Nexnexcoin-related debug output
            if(ptrCategory->count(string("nexnexcoin"))) {
...
#else
    const char* pszModule = "nexnexcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NexnexcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NexnexcoinCore
    // Mac: ~/Library/Application Support/NexnexcoinCore
    // Unix: ~/.nexnexcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NexnexcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NexnexcoinCore";
#else
    // Unix
    return pathRet / ".nexnexcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty nexnexcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override nexnexcoin.conf
