...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Nexnexcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Nexnexcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Nexnexcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Nexnexcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Nexnexcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Nexnexcoin 0.8 datadir."
    ;;
esac
